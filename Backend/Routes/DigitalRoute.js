...
const DigitalSchema =require("../Databases/DigitalModule");
...
router.get("/view", async (req,res)=>{
    try{
        const digitalcollection = await DigitalSchema.find();
        res.json(digitalcollection);
    } catch(error){
...
router.get("/view/:id", async (req, res) => {
    try {
        const digital = await DigitalSchema.findById(req.params.id);
        if (!digital) {
